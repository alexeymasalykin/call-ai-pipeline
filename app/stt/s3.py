...
class S3Client:
    def __init__(self, bucket: str, access_key: str, secret_key: str, endpoint: str) -> None:
        self._bucket = bucket
        self._s3 = boto3.client("s3", endpoint_url=endpoint, aws_access_key_id=access_key, aws_secret_access_key=secret_key)

    async def upload(self, local_path: Path, s3_key: str) -> str:
        loop = asyncio.get_running_loop()
        for attempt, delay in enumerate(_RETRY_DELAYS + [None], start=1):
            try:
                await loop.run_in_executor(None, partial(self._s3.upload_file, str(local_path), self._bucket, s3_key))
    ...
    async def delete(self, s3_key: str) -> None:
        loop = asyncio.get_running_loop()
        try:
            await loop.run_in_executor(None, partial(self._s3.delete_object, Bucket=self._bucket, Key=s3_key))
